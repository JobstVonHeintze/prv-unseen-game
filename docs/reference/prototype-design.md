...
| Condition | `condition.<slug>` | `condition.wetness` |
| Incident | `incident.<slug>` | `incident.rain-street` |
...
### 2.9 Writer drop-in (conditions, incidents, conduct)

A writer places these anywhere a choice or a secret use already exists. No
engine change. Only one incident may be open at a time.

- **Condition:** YAML under `canon/conditions/`. Felt lines only; never a HUD.
- **Incident:** YAML under `canon/incidents/` plus a cover scene. Start it with
  `{ start_incident: incident.<slug> }` on a choice, or list the choice under
  `after_choice`. Invalid cover choices add pressure and may step a condition.
  `valid` exits resolve it and may fire a gate.
- **Follow-up scene:** `{ enter: scene.<slug> }` on a choice or on
  `uses.leverage` / `tell` / `keep` effects. Same for `{ bank: secret.<id> }`.
- **Spice:** optional `spice: {l1,l2,l3}` on the scene or secret. State does
  not fork. L3 is hand-written or a placeholder; the Player falls back to l2.
- **Conduct as consequence:** a taken secret `about` that character, recipient
  classifies the use. A demand menu is a follow-up scene, not a second use type.
